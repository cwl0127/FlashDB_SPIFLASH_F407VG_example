...
void tsdb_sample(fdb_tsdb_t tsdb)
{
    ...
    { /* APPEND new TSL (time series log) */
        ...
        FDB_INFO("append the new status.temp (%d) and status.humi (%d), err=%u\n", status.temp, status.humi, err);
    ...
    { /* QUERY the TSDB by time */
        ...
        struct tm tm_from = {.tm_sec = 0, .tm_min = 0, .tm_hour = 0, .tm_mday = 1, .tm_mon = 0, .tm_year = 1970 - 1900};
        struct tm tm_to = {.tm_sec = 0, .tm_min = 0, .tm_hour = 0, .tm_mday = 5, .tm_mon = 4, .tm_year = 2020 - 1900 };
...
static bool query_cb(fdb_tsl_t tsl, void *arg)
{
    ...
    FDB_INFO("[query_cb] queried a TSL: time: %lld, temp: %d, humi: %d, len= %u\n", tsl->time, status.temp, status.humi, len);
...
static bool query_by_time_cb(fdb_tsl_t tsl, void *arg)
{
    ...
    FDB_INFO("[query_by_time_cb] queried a TSL: time: %lld, temp: %d, humi: %d\n", tsl->time, status.temp, status.humi);
...
static bool set_status_cb(fdb_tsl_t tsl, void *arg)
{
    ...
    FDB_INFO("set the TSL (time %lld) status from %d to %d\n", tsl->time, tsl->status, FDB_TSL_USER_STATUS1);
